 ...
 */
@RestController
@RequestMapping("/uploadx")

public class HjjUploadMeoth_good {
    ...
    public Map<String, Object> uploadDataset(@RequestParam("file") MultipartFile file, String filePath, HttpServletRequest request, Map<String, String> map) throws Exception {
        ...
        if (null == filePath) {  //单独算法上传
            ...
            //基本信息：
            visible_img_img_classfication = request.getParameter("isible_light_img");
            visible_img_target_detection = request.getParameter("isible_light_detection");
            visible_img_target_trace = request.getParameter("isible_light_tracking");
            infrared_ray_img_classfication = request.getParameter("red_outer_img");
            infrared_ray_target_detection = request.getParameter("red_outer_detection");
            infrared_ray_target_trace = request.getParameter("red_outer_tracking");
            text_sen_analysis = request.getParameter("text_analysis");
            text_entity_recognition = request.getParameter("text_named_entity");
            asr_asr_recognition = request.getParameter("voicle_recognition");

            //行人重识别
            visible_img_img_reid = request.getParameter("visible_img_img_reid");
            //语义分割
            visible_img_img_segmentation = request.getParameter("visible_img_img_segmentation");
            //视频-目标检测
            video_target_detection = request.getParameter("video_target_detection");
            //视频-目标跟踪
            video_target_trace = request.getParameter("video_target_trace");
            //结构化数据分类
            structured_data_classification = request.getParameter("structured_data_classification");
            //结构化数据回归
            structured_data_regression = request.getParameter("structured_data_regression");
            ...
            api = map.get("api");
            type = map.get("type");
            if (StrUtil.equals(type, MethodType.Text_Sen_Analysis)) {   //文本-情感分析    tex   text.sen_analysis
        ...
        //3.文本-情感分析    //text_sen_analysis_dataset    text.sen_analysis   text_sen_analysis
        ...
        //9.可见光-行人重识别
        if (StrUtil.equals(visible_img_img_reid, MethodType.Visible_Img_Img_Reid)) {
           // String Text_Sen_Analysis_JsonFileName = "datasetupload/datasetModel.json";
            String rename_schemeName = null;
            if (StrUtil.isNotEmpty(reDdatasetName)) {
                rename_schemeName = reDdatasetName;
        ...
        //10.可见光-语义分割
        if (StrUtil.equals(visible_img_img_segmentation, MethodType.Visible_Img_Img_Segmentation)) {
            // String Text_Sen_Analysis_JsonFileName = "datasetupload/datasetModel.json";
            String rename_schemeName = null;
            if (StrUtil.isNotEmpty(reDdatasetName)) {
                rename_schemeName = reDdatasetName;
        ...
        //10.视频-目标检测
        if (StrUtil.equals(video_target_detection, MethodType.Video_Detection)) {
            // String Text_Sen_Analysis_JsonFileName = "datasetupload/datasetModel.json";
            String rename_schemeName = null;
            if (StrUtil.isNotEmpty(reDdatasetName)) {
                rename_schemeName = reDdatasetName;
        ...
        //11.视频-目标跟踪
        if (StrUtil.equals(video_target_trace, MethodType.Video_Target_Trace)) {
            // String Text_Sen_Analysis_JsonFileName = "datasetupload/datasetModel.json";
            String rename_schemeName = null;
            if (StrUtil.isNotEmpty(reDdatasetName)) {
                rename_schemeName = reDdatasetName;
        ...
        //11.结构化数据分类
        if (StrUtil.equals(structured_data_classification, MethodType.Structured_Data_Classification)) {
            // String Text_Sen_Analysis_JsonFileName = "datasetupload/datasetModel.json";
            String rename_schemeName = null;
            if (StrUtil.isNotEmpty(reDdatasetName)) {
                rename_schemeName = reDdatasetName;
        ...
        //12.结构化数据回归
        if (StrUtil.equals(structured_data_regression, MethodType.Structured_Data_Regression)) {
            // String Text_Sen_Analysis_JsonFileName = "datasetupload/datasetModel.json";
            String rename_schemeName = null;
            if (StrUtil.isNotEmpty(reDdatasetName)) {
                rename_schemeName = reDdatasetName;
    ...
    public void uploadUnify(@RequestParam("file") MultipartFile file, String filePath, HttpServletRequest request, Map<String, String> map) throws Exception {
        ...
        if (null == filePath) {  //单独算法上传
            ...
            if(StrUtil.isNotEmpty(visible_img_img_classfication)){
                type = visible_img_img_classfication;
            }else if(StrUtil.isNotEmpty(visible_img_target_detection)){
                type = visible_img_target_detection;
            }else if(StrUtil.isNotEmpty(visible_img_target_trace)){
                type = visible_img_target_trace;
            }else if(StrUtil.isNotEmpty(visible_img_img_reid)){
                type = visible_img_img_reid;
            }else if(StrUtil.isNotEmpty(visible_img_img_segmentation)){
                type = visible_img_img_segmentation;
            }else if(StrUtil.isNotEmpty(infrared_ray_img_classfication)){
                type = infrared_ray_img_classfication;
            }else if(StrUtil.isNotEmpty(infrared_ray_target_detection)){
                type = infrared_ray_target_detection;
            }else if(StrUtil.isNotEmpty(infrared_ray_target_trace)){
                type = infrared_ray_target_trace;
            }else if(StrUtil.isNotEmpty(video_target_detection)){
                type = video_target_detection;
            }else if(StrUtil.isNotEmpty(video_target_trace)){
                type = video_target_trace;
            }else if(StrUtil.isNotEmpty(text_sen_analysis)){
                type = text_sen_analysis;
            }else if(StrUtil.isNotEmpty(text_entity_recognition)){
                type = text_entity_recognition;
            }else if(StrUtil.isNotEmpty(asr_asr_recognition)){
                type = asr_asr_recognition;
            }else if(StrUtil.isNotEmpty(structured_data_classification)){
                type = structured_data_classification;
            }else if(StrUtil.isNotEmpty(structured_data_regression)){
                type = structured_data_regression;
            }else if(StrUtil.isNotEmpty(structured_data_cluster)){
        ...
        //3.图像分类： img_classfication (可见光-图像分类、红外线-图像分类 都有相同结尾)
        if (StrUtil.contains(MethodType.Img_Classfication, type)) {
            log.info("图像分类上传");
            ...
            //1.添加图像分类,如果类型与上传类型不一致则不上传
            if (fileName.contains("visible_img_img")) {     //可见光
                type_list.add(MethodType.Visible_Img_Img_Classfication);
            } else if (fileName.contains("infrared_ray")) {   //红外线
                type_list.add(MethodType.Infrared_Ray_Img_Classfication);
                log.info("上传的是红外线图像分类算法");
            } else {  //两者都上传
                System.out.println("两者都");
                // type_list.add(MethodType.Visible_Img_Img_Classfication);
                // type_list.add(MethodType.Infrared_Ray_Img_Classfication);
            }
        ...
        //4.目标检测   target_detection   (可见光-目标检测、红外线-目标检测、视频-目标检测)
        if (StrUtil.contains(MethodType.Target_Detection, type)) {
            log.info("目标检测上传");
            List<String> method_type = new ArrayList<>();
            method_type.add("black");
            method_type.add("white");
            List<String> type_list = new ArrayList<>();
            //1.添加目标检测,如果类型与上传类型不一致则不上传
            if (fileName.contains("visible_img")) {  //可见光-目标检测
                type_list.add(MethodType.Visible_Img_Target_Detection);
            } else if (fileName.contains("infrared_ray")) {  //红外线-目标检测
                type_list.add(MethodType.Infrared_Ray_Target_Detection);
            } else if (fileName.contains("video".toLowerCase())) {   //Video_Detection  //视频-目标检测
        ...
        //6.文本-情感  text.sen_analysis
        if (StrUtil.contains(MethodType.Text_Sen_Analysis, type)) {
            log.info("文本-情感上传");
            List<String> method_type = new ArrayList<>();
            method_type.add("black");
            // method_type.add("white");
        ...
        //7.文本-识别  text.entity_recognition
        if (StrUtil.contains(MethodType.Text_Entity_Recognition, type)) {
            log.info("文本-识别上传");
            List<String> method_type = new ArrayList<>();
            method_type.add("black");
            //  method_type.add("white");
        ...
        //8.语音-识别  asr.asr_recognition
        if (StrUtil.contains(MethodType.Asr_Asr_Recognition, type)) {
            log.info("语音-识别上传");
            List<String> method_type = new ArrayList<>();
            //  method_type.add("black");
            method_type.add("white");
        ...
        //9.目标跟踪   target_trace  (可见光-目标跟踪、红外线-目标跟踪、视频-目标跟踪)
        if (StrUtil.contains(MethodType.Target_Trace, type)) {
            log.info("目标跟踪上传");
            List<String> method_type = new ArrayList<>();
            method_type.add("black");
            method_type.add("white");
        ...
        //9.行人重识别   visible_img.img_reid  (可见光-行人重识别)
        if (StrUtil.contains(MethodType.Visible_Img_Img_Reid, type)) {
            log.info("行人重识别算法上传");
            List<String> method_type = new ArrayList<>();
            method_type.add("black");
            method_type.add("white");
        ...
        //10.语义分割   visible_img.img_segmentation  (可见光-语义分割)
        if (StrUtil.contains(MethodType.Visible_Img_Img_Segmentation, type)) {
            log.info("行人重识别算法上传");
            List<String> method_type = new ArrayList<>();
            method_type.add("black");
            method_type.add("white");
        ...
        //11.结构化分类   structured_data.classification  (结构化数据-分类)
        if (StrUtil.contains(MethodType.Structured_Data_Classification,type)) {
            log.info("行人重识别算法上传");
            List<String> method_type = new ArrayList<>();
            method_type.add("black");
            method_type.add("white");
            ...
            //1.添加图像分类,如果类型与上传类型不一致则不上传
            if (fileName.contains("classification")) {     //结构化数据-分类
                   type_list.add(MethodType.Structured_Data_Classification);
            } else if (fileName.contains("regression")) {   //结构化数据-回归
        ...
        //12.结构化回归   structured_data.classification  (结构化数据-回归)
        if (StrUtil.contains(MethodType.Structured_Data_Regression,type)) {
            log.info("行人重识别算法上传");
            List<String> method_type = new ArrayList<>();
            method_type.add("black");
            method_type.add("white");
